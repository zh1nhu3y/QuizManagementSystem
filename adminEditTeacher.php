...
Description    : Admin Edit Teacher Page
 -->
<?php
session_start();
include 'dbConn.php';
...
if(isset($_POST['update'])){
    $tfirstname = $_POST['txtTFName'];
    $tlastname = $_POST['txtTLName'];
    $tphonenumber = $_POST['txtTPhoneNumber'];
    $temail = $_POST['txtTEmail'];
    $updateQuery = "UPDATE `teacher_t` SET `tcr_fn`='$tfirstname',`tcr_ln`='$tlastname',`tcr_pn`='$tphonenumber',`tcr_em`='$temail' WHERE tcr_id='$myID'";
...
$query = "SELECT * FROM teacher_t WHERE tcr_id = " . $myID;
...
<head>
    ...
    <title>Admin Teacher Edit</title>
    ...
    <section>
        ...
        <div class="div1">
            <form action="" method="post">
                ...
                <input type="text" id="tfname" name="txtTFName" placeholder="Your name.." value="<?php echo $row['tcr_fn']; ?>" pattern="[A-Za-z\s]+" required>
                <br>
                <label for="tlname">Teacher Last Name:</label>
                <input type="text" id="slname" name="txtTLName" placeholder="Your last name.." value="<?php echo $row['tcr_ln']; ?>" pattern="[A-Za-z\s]+" required>
                <br>
                <label for="tphone">Teacher Phone Number:</label>
                <input type="text" id="tphone" name="txtTPhoneNumber" placeholder="0123456789" value="<?php echo $row['tcr_pn']; ?>" pattern="[0-9]{10,11}" required>
                <br>
                <label for="temail">Teacher Email:</label>
                <input type="text" id="temail" name="txtTEmail" placeholder="Your email.." value="<?php echo $row['tcr_em']; ?>" required>
                <br>
